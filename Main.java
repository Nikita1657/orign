...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        if (action == ' ') throw new Exception("Некорректный знак ");
        String[] data = exp.split(" \\" + action + " ");
        if (data[0].length() > 40 || data[1].length() > 10) throw new Exception("Введено больше 10 символов");
        if (action == '*' || action == '/') {
            if (data[1].contains("\"")) throw new Exception("Строку можно делить/умножать только на число");
        }
    ...
    static String result(char action, String[] data) {
        return switch (action) {
            ...
            default -> throw new IllegalArgumentException("не првильный аргумент");
        };
    }
